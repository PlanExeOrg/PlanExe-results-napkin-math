...
```json
{
  ...
  "artifact_set": {
    ...
    "relative_dir": "output/v49/20251114_paperclip_automation"
  },
  "source_plan_dir": "/Users/neoneye/git/PlanExeGroup/PlanExe/experiments/napkin_math/output/v49/20251114_paperclip_automation",
...
- **Fragile** — `manual_intervention_margin_hours` holds in only 29.3% of simulated runs under the current input bounds (fails in the majority).
...
|---|---|---|---|---:|---|---|
|  | `manual_intervention_margin_hours` | >= 0.0000 | report_explicit | 29.3% | **Fragile** | fails in the majority of runs |
|  | `budget_surplus_usd` | >= 0.0000 | report_explicit | 72.6% | **Marginal** | passes more often than not but uncomfortably close |
|  | `api_latency_margin_ms` | >= 0.0000 | report_explicit | 62.1% | **Marginal** | passes more often than not but uncomfortably close |
|  | `opc_ua_bid_margin_usd` | >= 0.0000 | report_explicit | 86.7% | **Robust** | passes in the strong majority of runs |
|  | `onsite_expert_budget_margin_usd` | >= 0.0000 | report_explicit | 86.4% | **Robust** | passes in the strong majority of runs |
...
|---|---|---|---|---|
| `manual_intervention_margin_hours` | **Fragile** | The `>= 0.0000` requirement fails in the majority of runs (70.7%). External commitments built on this gate are exposed. | `actual_manual_intervention_hours_per_week` (reduce toward its low bound; quartile Δ-pp -100.0) | Positive margin clears the feasibility-proof success metric; negative margin invalidates the central Builder thesis regardless of how clean the integration is otherwise. Threshold parameter: `manual_intervention_threshold_hours_per_week`. |
| `budget_surplus_usd` | **Marginal** | The `>= 0.0000` requirement passes 72.6% of runs — just below the Robust band. The gate passes in most runs, but downstream commitments should not treat it as secure. | `actual_total_project_cost_usd` (reduce toward its low bound; quartile Δ-pp -100.0) | Positive surplus preserves the Risk-3 contingency and Issue-1 spare-parts reserve; negative surplus forces scope cuts or sponsor capital call. Threshold parameter: `budget_max_usd`. |
| `api_latency_margin_ms` | **Marginal** | The `>= 0.0000` requirement passes 62.1% of runs — close to coin-flip. Downstream commitments should not assume it holds. | `actual_api_p99_latency_ms` (reduce toward its low bound; quartile Δ-pp -100.0) | Positive margin preserves the API-only Phase-6 demonstration target; negative margin forces HMI-controlled fallback and invalidates the autonomous-flow feasibility claim. Threshold parameter: `api_latency_p99_threshold_ms`. |
...
|---|---|---:|---|
| `manual_intervention_margin_hours` | `actual_manual_intervention_hours_per_week` | -100.0 | `actual_manual_intervention_hours_per_week` below p67 |
...
|---:|---|---|---:|---:|---|
| 1 | `actual_manual_intervention_hours_per_week` | `manual_intervention_margin_hours` | 123.69 | 1.75 | report_derived |
| 2 | `actual_api_p99_latency_ms` | `api_latency_margin_ms` | 83.86 | 2.21 | model_assumption |
| 3 | `actual_total_project_cost_usd` | `budget_surplus_usd` | 15.08 | 0.55 | report_derived |
| 4 | `actual_onsite_expert_cost_usd` | `onsite_expert_budget_margin_usd` | 10.90 | 1.47 | report_derived |
| 5 | `actual_opc_ua_bid_usd` | `opc_ua_bid_margin_usd` | 9.14 | 1.30 | report_derived |
...
|---|:---:|---:|---:|
| `manual_intervention_margin_hours` | **LOW** | 100% | 1.75 |
| `budget_surplus_usd` | **MEDIUM** | 100% | 0.55 |
| `opc_ua_bid_margin_usd` | **MEDIUM** | 100% | 1.30 |
| `api_latency_margin_ms` | **LOW** | 0% | 2.21 |
| `onsite_expert_budget_margin_usd` | **MEDIUM** | 100% | 1.47 |
...
|---|---|---:|---:|---:|
| `manual_intervention_margin_hours` | hours_per_week | 1.00 | 0.0000 | -2.50 |
| `budget_surplus_usd` | USD | 200,000 | 0.0000 | -75,000 |
| `opc_ua_bid_margin_usd` | USD | 45,000 | 25,000 | -20,000 |
| `api_latency_margin_ms` | milliseconds | 75.00 | 30.00 | -80.00 |
| `onsite_expert_budget_margin_usd` | USD | 15,000 | 10,000 | -7,000 |

## Suggested next actions

Imperatives for whatever consumes this file next.

1. To answer whether the plan is viable, lead with the gate verdicts above — not the source plan's narrative. 1 in the Fragile band. Worst: `manual_intervention_margin_hours` at 29.3% pass rate under current bounds.
