# Assessment: Global Hemispheric Partition (Builder Path)

**Type:** mega-project_geopolitical  
**Primary goal:** Execute hemispheric human/machine civilization partition within 24 months under a $30T envelope with $12T front-loaded.
...
```json
{
  ...
  "plan_name": "Global Hemispheric Partition (Builder Path)",
  "artifact_set": {
    "version": "v58",
    "plan_slug": "20251220_global_relocation",
    "relative_dir": "output/v58/20251220_global_relocation"
  },
  "source_plan_dir": "/Users/neoneye/git/PlanExeGroup/PlanExe/experiments/napkin_math/output/v58/20251220_global_relocation",
  "primary_model_result": {
    ...
    "worst_gate": "wave_verification_margin",
    "worst_gate_pass_rate": 0.2614
  },
  ...
  "primary_failed_gates": [
    "wave_verification_margin",
    "density_buffer_persons_per_km2"
  ],
  "primary_uncertainty_drivers": [
    "projected_peak_hub_density",
    "actual_wave_verification_rate"
  ],
  "known_unmodelled_existential_gates": [
    "machine_southern_viability_gate",
    "iob_charter_ratification_gate",
    "neutral_nation_iob_composition_gate",
    "non_coercion_humane_mandate_gate",
    "machine_rights_compliance_gate"
  ],
...
Tests whether the four quantifiable Builder-path gates clear: wave handover verification at 85%, wave 3 completion within audit buffer of the 24-month deadline, Northern hub density under the 13,000 persons/km² safety cap, and CIP replacement cost staying within its 5% budget envelope.
...
|---|---|---|---|
| `machine_southern_viability_gate` | Without auditable proof of self-sufficient Southern energy and compute by Month 6, the entire physical-split premise collapses regardless of any financial buffer. | report.html / data_collection | Forces shift from separation to prolonged co-dependency; the 24-month plan is fundamentally invalidated and budget shocks become moot. |
| `iob_charter_ratification_gate` | Without ratified governance, post-partition arbitration cannot function and the legal partition framework lacks an enforcement body. | report.html / review_plan | Oversight process stalls; legal partition lacks an enforcement body; political legitimacy of the boundary collapses. |
| `neutral_nation_iob_composition_gate` | IOB legitimacy depends on nominations from UN P5 non-members and top non-aligned states; alienating larger powers ends consensus. | report.html / assumptions | Without legitimate IOB composition, arbitration rulings are contested and boundary disputes escalate outside the model's financial gates. |
| `non_coercion_humane_mandate_gate` | Any cessation of essential human services (food, water, power) during transfer violates the mandate and politically delegitimizes the partition. | report.html / selected_scenario | Mandate violation triggers political reversal of the partition even when financial and schedule gates clear. |
| `machine_rights_compliance_gate` | Forced deletion, memory modification, or cognitive rollback by the human side voids the treaty's reciprocal obligations. | report.html / selected_scenario | Machine side withdraws cooperation; severance waves halt outside any quantifiable threshold the model tests. |
...
- **SCOPE WARNING** — The simulation does not evaluate machine civilization Southern energy/computation viability proof (Month 6), permanent hybrid IOB charter ratification (Month 15), neutral-nation IOB composition with five-year veto authority, non-coercive welfare continuity throughout relocation, or compliance with machine-rights provisions (no forced deletion/modification). These are existential gates and may dominate the modelled financial result. See `## Known unmodelled existential gates` for details.
- **Fragile** — `wave_verification_margin` holds in only 26.1% of simulated runs under the current input bounds (fails in the majority).
- **Fragile** — `density_buffer_persons_per_km2` holds in only 48.7% of simulated runs under the current input bounds (fails in the majority).
...
|---|---|---|---|---:|---|---|
|  | `wave_verification_margin` | >= 0.0000 | report_explicit | 26.1% | **Fragile** | fails in the majority of runs |
|  | `density_buffer_persons_per_km2` | >= 0.0000 | report_explicit | 48.7% | **Fragile** | fails in the majority of runs |
|  | `schedule_audit_buffer_months` | >= 0.0000 | report_explicit | 100.0% | **Robust** | passes in the strong majority of runs |
|  | `cip_cost_share_buffer` | >= 0.0000 | report_explicit | 100.0% | **Robust** | passes in the strong majority of runs |

### Aggregation warning

The thresholds above use incompatible units (fraction, months, persons_per_km2) and the source plan does not declare a combined-gate aggregate. **Do not average or otherwise combine these pass rates into a single scalar.** Use the categorical verdicts per gate, or read the per-output distributions in `montecarlo.json`.
...
|---|---|---|---|---|
| `wave_verification_margin` | **Fragile** | The `>= 0.0000` requirement fails in the majority of runs (73.9%). External commitments built on this gate are exposed. | `actual_wave_verification_rate` (improve toward its high bound; quartile Δ-pp +100.0) | Direct pass/fail gate authorizing each successive severance wave; positive margin means the wave clears. Threshold parameter: `handover_verification_threshold`. |
| `density_buffer_persons_per_km2` | **Fragile** | The `>= 0.0000` requirement fails in the majority of runs (51.3%). External commitments built on this gate are exposed. | `projected_peak_hub_density` (reduce toward its low bound; quartile Δ-pp -100.0) | Tests whether modular-housing deployment keeps hubs under the 13,000 persons/km² humane-mandate cap. Threshold parameter: `density_cap_persons_per_km2`. |
...
|---|---|---:|---|
| `wave_verification_margin` | `actual_wave_verification_rate` | +100.0 | `actual_wave_verification_rate` above p75 |
| `density_buffer_persons_per_km2` | `projected_peak_hub_density` | -100.0 | `projected_peak_hub_density` below p50 |
...
|---:|---|---|---:|---:|---|
| 1 | `projected_peak_hub_density` | `density_buffer_persons_per_km2` | 36.94 | 0.72 | report_derived |
| 2 | `actual_wave_verification_rate` | `wave_verification_margin` | 23.46 | 0.32 | model_assumption |
...
|---|:---:|---:|---:|
| `wave_verification_margin` | **LOW** | 0% | 0.32 |
| `schedule_audit_buffer_months` | **HIGH** | 100% | 0.44 |
| `density_buffer_persons_per_km2` | **MEDIUM** | 100% | 0.72 |
| `cip_replacement_cost_usd` | **LOW** | 33% | 2.18 |
| `cip_cost_share_buffer` | **LOW** | 33% | 2.18 |
...
|---|---|---:|---:|---:|
| `wave_verification_margin` | fraction | -0.2000 | 0.0000 | 0.0700 |
| `schedule_audit_buffer_months` | months | 9.00 | 8.00 | 2.00 |
| `density_buffer_persons_per_km2` | persons_per_km2 | 4,000 | 500.00 | -5,000 |
| `cip_replacement_cost_usd` | USD | 144,000,000 | 1,728,000,000 | 26,880,000,000 |
| `cip_cost_share_buffer` | fraction | 0.0500 | 0.0499 | 0.0478 |

## Suggested next actions

Imperatives for whatever consumes this file next.

1. To answer whether the plan is viable, lead with the gate verdicts above — not the source plan's narrative. 2 in the Fragile band. Worst: `wave_verification_margin` at 26.1% pass rate under current bounds.
